...
export function SelectTrigger({ className, ...props }: SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger
      className={twMerge(
        'px-3 py-1 text-zinc-100 tabular-nums rounded-md border border-zinc-800 bg-zinc-800/50 flex items-center gap-2.5',
        className,
      )}
      {...props}
    >
...
export function SelectItem({ className, children, ...props }: SelectItemProps) {
  return (
    <SelectPrimitive.Item
      className={twMerge(
        'flex items-center text-sm gap-2 text-zinc-300 px-3 py-1.5 justify-between outline-none hover:bg-zinc-800',
        className,
      )}
      {...props}
    >
